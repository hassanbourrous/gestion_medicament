<?php
namespace App\Policies;

use App\Models\StockHistory;
use App\Models\User;
use Illuminate\Auth\Access\HandlesAuthorization;

class StockHistoryPolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the user can view any models.
     *
     * @param    \App\Models\User  $user
     * @return  mixed
     */
    public function viewAny(User $user)
    {
        return $user->hasPermissionTo('stock-histories.index');
    }

    /**
     * Determine whether the user can view the model.
     *
     * @param    \App\Models\User  $user
     * @param    StockHistory  $stockHistory
     * @return  mixed
     */
    public function view(User $user, StockHistory $stockHistory)
    {
        return $user->hasPermissionTo('stock-histories.show');
    }

    /**
     * Determine whether the user can create models.
     *
     * @param    \App\Models\User  $user
     * @return  mixed
     */
    public function create(User $user)
    {
        return $user->hasPermissionTo('stock-histories.create');
    }

    /**
     * Determine whether the user can update the model.
     *
     * @param    \App\Models\User  $user
     * @param    StockHistory  $stockHistory
     * @return  mixed
     */
    public function update(User $user, StockHistory $stockHistory)
    {
        return $user->hasPermissionTo('stock-histories.edit');
    }

    /**
     * Determine whether the user can delete the model.
     *
     * @param    \App\Models\User  $user
     * @param    StockHistory  $stockHistory
     * @return  mixed
     */
    public function delete(User $user, StockHistory $stockHistory)
    {
        return $user->hasPermissionTo('stock-histories.delete');
    }

    /**
     * Determine whether the user can restore the model.
     *
     * @param    \App\Models\User  $user
     * @param    StockHistory  $stockHistory
     * @return  mixed
     */
    public function restore(User $user, StockHistory $stockHistory)
    {
        return false;
    }

    /**
     * Determine whether the user can permanently delete the model.
     *
     * @param    \App\Models\User  $user
     * @param    StockHistory  $stockHistory
     * @return  mixed
     */
    public function forceDelete(User $user, StockHistory $stockHistory)
    {
        return $user->hasPermissionTo('stock-histories.delete');
    }
}
