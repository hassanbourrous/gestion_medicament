    ...
    */
    public function edit(Request $request, User $user)
    {
        try {
            ...
            //Fetch relationships
            $establishments = Establishment::all(['id','name','region_id'])->map(function($estab) use($user) {
                $estab->checked = $user->establishments->contains($estab);
                $estab->append('groupe');
                return $estab->toArray();
            })->groupBy('groupe');
            return Inertia::render("Users/Edit", ["model" => $user,"establishments" => $establishments, "roles"=> $roles]);
        } catch (\Throwable $exception) {
            \Log::error($exception);
            return back()->with([
                'error' => $exception->getMessage(),
