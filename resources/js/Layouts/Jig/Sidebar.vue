<template>
    ...
    >
        ...
        >
            ...
            <application-logo class="h-full px-2 bg-primary m-auto" />
            ...
            >
                ...
                <ul class="flex flex-col list-none md:flex-col md:min-w-full">
                    <li v-for="(menuItem, idx) of menuItems" :key="idx" class="items-center">
                        ...
                        <jig-sidebar-link v-else-if="menuItem.isParent && ($page.props.menu_permissions[idx] || !!menuItem.ignorePerm)"
                            class="flex flex-wrap items-center justify-between" @click.native.prevent="toggleExpanded(idx)" :active="false">
                            <div class="text-md">
                        ...
                        <ul v-if="menuItem.isParent && ($page.props.menu_permissions[idx] || menuItem.ignorePerm) && expanded === idx" class="flex-col pl-2 list-none md:flex-col md:min-w-full">
                            <li v-for="(child,childIdx) of menuItem.children" :key="`${idx}-${childIdx}`" class="items-center">
                                ...
                                <jig-sidebar-link v-else-if="child.isParent && ($page.props.menu_permissions[childIdx] || !!child.ignorePerm)"  
                                    ...
                                    <i :class="`mr-2 pl-4 text-md ml-auto text-gray-500 fas ${expanded2 === idx+'-'+childIdx ? 'fa-angle-down' : 'fa-angle-right'}`"></i>
                                </jig-sidebar-link>
...
export default defineComponent({
    ...
    methods: {
        ...
        toggleExpanded2(target) {
            if (target ===this.expanded2) {
                this.expanded2 = null;
            } else {
                this.expanded2 = target;
            }
        },
