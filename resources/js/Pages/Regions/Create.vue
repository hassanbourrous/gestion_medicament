    ...
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                <inertia-link :href="route('admin.regions.index')"
                              class="text-xl font-black text-white"><i
                        class="fas fa-arrow-left"></i> Back | New Region
                </inertia-link>
            </div>
        </template>
        <div class="flex flex-wrap px-4">
            <div class="z-10 flex-auto max-w-2xl p-4 mx-auto bg-white md:rounded-md md:shadow-md">
                <create-regions-form @success="onSuccess" @error="onError"/>
            </div>
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import CreateRegionsForm from "./CreateForm.vue";
