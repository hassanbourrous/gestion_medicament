    ...
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.permissions.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> New
                        Permission</inertia-button>
        ...
        <div v-if="can.viewAny" class="flex flex-wrap px-4">
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                <h3 class="w-full p-4 mb-2 text-lg font-black sm:rounded-t-lg bg-primary-100"><i class="mr-2 fas fa-bars"></i> List des Permissions</h3>
                <div class="p-4">
                    <dt-component
                        :table-id="tableId"
                        :ajax-url="ajaxUrl"
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Permission #{{currentModel.id}}</template>
                        <show-permissions-form :model="currentModel"></show-permissions-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Permissions
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowPermissionsForm from "@/Pages/Permissions/ShowForm.vue";
    export default {
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.permissions.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.permissions.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.permissions.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.permissions.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.permissions.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.permissions.update`,model.id),{
                    enabled: enabled
                }).then(res => {
